...
import { SentencePair, canDeleteTranslatedItem, getDeleteTimeRemaining } from '../types';
...
export default function SentenceList({
  ...
  useEffect(() => {
    const hasRecentTranslations = sentences.some(
      (sentence) => sentence.tamil?.trim() && getDeleteTimeRemaining(sentence.updated_at) > 0
    );
  ...
  // Check if a sentence can be deleted
  const canDelete = (sentence: SentencePair): boolean => {
    // Sentences without Tamil translations can always be deleted
    if (!sentence.tamil?.trim()) return true;
    // Sentences with translations can only be deleted within 3-minute window
    return canDeleteTranslatedItem(sentence.updated_at);
  };
